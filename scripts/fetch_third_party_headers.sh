...
ZSTD_INCLUDE_ROOT="${ROOT_DIR}/Dependicies/Sources/zstd/include"

if [[ -f "${ZSTD_INCLUDE_ROOT}/zstd.h" && -f "${ZSTD_INCLUDE_ROOT}/seekable_format/zstd_seekable.h" ]]; then
  echo "Zstd headers already present: ${ZSTD_INCLUDE_ROOT}"
else
  echo "Fetching Zstd headers ${ZSTD_VERSION}..."
  curl -L "${ZSTD_URL}" -o "${TMP_DIR}/${ZSTD_TAR}"
  tar -xzf "${TMP_DIR}/${ZSTD_TAR}" -C "${TMP_DIR}"

  mkdir -p "${ZSTD_INCLUDE_ROOT}/seekable_format"
  cp "${TMP_DIR}/zstd-${ZSTD_VERSION}/lib/zstd.h" "${ZSTD_INCLUDE_ROOT}/"
  cp "${TMP_DIR}/zstd-${ZSTD_VERSION}/lib/zdict.h" "${ZSTD_INCLUDE_ROOT}/"
  cp "${TMP_DIR}/zstd-${ZSTD_VERSION}/lib/zstd_errors.h" "${ZSTD_INCLUDE_ROOT}/"
  cp "${TMP_DIR}/zstd-${ZSTD_VERSION}/contrib/seekable_format/zstd_seekable.h" "${ZSTD_INCLUDE_ROOT}/seekable_format/"
  echo "Installed Zstd headers to ${ZSTD_INCLUDE_ROOT}"
fi
